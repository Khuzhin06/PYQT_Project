...
from project_final3 import Ui_Dialog
...
class Draw_maker(QMainWindow, Ui_Dialog):
    def __init__(self):
        ...
        self.spin = {self.tree: [self.spinBox_tree],
                     ...
                     self.checkBox_house: [self.spinBox_house],
    ...
    def make_house(self, gde_y, color='#855a1d', color_2='#851d22', color_3='#543310', color_4='#24ede6'):
        if self.checkBox_house.checkState():
            ...
            if self.radioButton_ocean.isChecked():
                a = a // 3
            ...
            if randint(0, 1):
                ...
                self.drawer.rectangle(((gde_x - a * 3.25, gde_y - a * 2.75), (gde_x - a * 1.75, gde_y - a * 1.25)),
                                      color_4)
    ...
    def make_draw(self):
        ...
        try:
            self.im.save(self.filename.toPlainText())
            self.second_form = Second_form(self.filename.toPlainText())
    ...
    def db(self, number, season=False, day=False):
        try:
            if season:  # Проверяем важен ли нам сезон года, или время дня
                if not day:
                    a = self.cur.execute(
                        "SELECT cod, day FROM color WHERE color = " + str(
                ...
                a = self.cur.execute(
                    "SELECT cod, day FROM color WHERE color = " + str(number) + " and (day = " + self.day_id
                    + ")").fetchall()
                result1, b = choice(a)
                return result1
        except Exception:
